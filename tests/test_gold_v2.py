...
def load_table(name: str) -> pd.DataFrame:
    ...
    assert path.exists(), f"Missing gold_v2 table: {path}"
...
def run_gold_v2_tests() -> None:
    ...
    assert_table_not_empty(dim_date, "dim_date")
    assert_columns_exist(
        dim_date,
        ["date_key", "full_date", "year", "month", "day", "day_name", "service_id"],
        "dim_date",
    )
    assert_no_nulls(dim_date, ["date_key", "full_date", "service_id"], "dim_date")
    assert_no_duplicates(dim_date, ["date_key", "service_id"], "dim_date")
    ...
    print("PASS: Gold V2 tests passed.")
